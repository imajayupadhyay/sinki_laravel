<template>
    <AdminLayout
        page-title="Weekly Databricks Management"
        page-subtitle="Manage weekly Databricks articles, drafts and published content"
    >
        <Head title="Weekly Databricks - Admin Dashboard" />
        ...
        <!-- Weekly Databricks Table -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <!-- Header -->
            <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    <h2 class="text-lg font-semibold text-gray-900">Weekly Databricks</h2>
                    <span class="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm font-medium">
                        {{ weeklyDatabricks.total }} {{ weeklyDatabricks.total === 1 ? 'article' : 'articles' }}
                    </span>
                </div>
                <Link
                    :href="route('admin.weekly-databricks.create')"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="weeklyDatabrick in weeklyDatabricks.data" :key="weeklyDatabrick.id" class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-4 min-w-0 flex-1">
                                        <div class="text-sm font-medium text-gray-900 truncate">{{ weeklyDatabrick.title }}</div>
                                        <div class="text-xs text-gray-500">{{ weeklyDatabrick.slug }}</div>
                                        <div v-if="weeklyDatabrick.tags" class="mt-1 flex flex-wrap gap-1">
                                            <span
                                                v-for="tag in weeklyDatabrick.tags.split(', ').slice(0, 3)"
                                                :key="tag"
                                            ...
                                            <span
                                                v-if="weeklyDatabrick.tags.split(', ').length > 3"
                                                class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-600"
                                            >
                                                +{{ weeklyDatabrick.tags.split(', ').length - 3 }} more
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    ...
                                    :class="{
                                        'bg-green-100 text-green-800': weeklyDatabrick.status === 'published',
                                        'bg-yellow-100 text-yellow-800': weeklyDatabrick.status === 'draft'
                                    }"
                                >
                                    {{ weeklyDatabrick.status === 'published' ? 'Published' : 'Draft' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ weeklyDatabrick.category || 'Uncategorized' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ weeklyDatabrick.author }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div v-if="weeklyDatabrick.published_at" class="text-sm text-gray-900">{{ weeklyDatabrick.published_at }}</div>
                                <div v-else class="text-xs text-gray-500">{{ weeklyDatabrick.created_at_human }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    <Link
                                        :href="route('admin.weekly-databricks.edit', weeklyDatabrick.id)"
                                    ...
                                    <Link
                                        :href="route('admin.weekly-databricks.preview', weeklyDatabrick.id)"
                                        target="_blank"
                ...
                <!-- Empty State -->
                <div v-if="!weeklyDatabricks.data || weeklyDatabricks.data.length === 0" class="text-center py-12">
                    <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    <h3 class="mt-4 text-lg font-medium text-gray-900">No weekly Databricks articles found</h3>
                    <p class="mt-2 text-gray-600">{{ hasActiveFilters ? 'Try adjusting your filters or' : '' }} Get started by creating your first weekly Databricks article.</p>
        ...
        <!-- Pagination -->
        <div v-if="weeklyDatabricks.data && weeklyDatabricks.data.length > 0" class="bg-white rounded-xl shadow-sm border border-gray-200 mt-6">
            <div class="px-6 py-4">
                <div class="flex items-center justify-between">
                    <div class="flex-1 flex justify-between sm:hidden">
                        <Link
                            v-if="weeklyDatabricks.prev_page_url"
                            :href="weeklyDatabricks.prev_page_url"
                            class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50"
                        >
                            Previous
                        </Link>
                        <Link
                            v-if="weeklyDatabricks.next_page_url"
                            :href="weeklyDatabricks.next_page_url"
                    ...
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">
                                Showing
                                <span class="font-medium">{{ weeklyDatabricks.from || 0 }}</span>
                                to
                                <span class="font-medium">{{ weeklyDatabricks.to || 0 }}</span>
                                of
                                <span class="font-medium">{{ weeklyDatabricks.total }}</span>
                                results
                            </p>
                        </div>
                        <div>
                            <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                                ...
                                <!-- Page Numbers -->
                                <template v-for="(link, index) in weeklyDatabricks.links" :key="index">
                                    <Link
        ...
        >
            <div class="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Delete Weekly Databricks Article</h3>
                <p class="text-gray-600 mb-6">
                    Are you sure you want to delete <strong>{{ selectedWeeklyDatabrick?.title }}</strong>? This action cannot be undone.
                </p>
                ...
                <div class="flex items-center justify-end space-x-3">
                    ...
                    >
                        {{ deleteForm.processing ? 'Deleting...' : 'Delete Article' }}
                    </button>
                </div>
            </div>
        </div>
...
const selectedWeeklyDatabrick = ref(null);
...
const applyFilters = () => {
    ...
    router.visit(route('admin.weekly-databricks.index'), {
        method: 'get',
        data: params,
        preserveState: true,
        preserveScroll: true
...
const closeDeleteModal = () => {
    ...
    selectedWeeklyDatabrick.value = null;
};

const deleteWeeklyDatabrick = () => {
    deleteForm.delete(route('admin.weekly-databricks.destroy', selectedWeeklyDatabrick.value.id), {
        onSuccess: () => {
            closeDeleteModal();
        }
    });
